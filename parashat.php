...
for ($i = 0; $i < count($arr['items']); $i++) {
  ...
  if ($arr['items'][$i]['category'] == "parashat") {
    ...
    if ($date >= $parashat_start && $date <= $parashat_end) {
      ...
      switch ($torah_trad[0]) {
        case 'Genesis':
          $parashat_torah = str_replace('Genesis', 'Bereshit/Gênesis', $torah_trad[0]);
          $parashat_array['parashat_torah'] = $parashat_torah;

          break;
        case 'Exodus':
          $parashat_torah = str_replace('Exodus', 'Shemot/Êxodo', $torah_trad[0]);
          $parashat_array['parashat_torah'] = $parashat_torah;
          break;

        case 'Leviticus':
          $parashat_torah = str_replace('Leviticus', 'Vayikra/Liviticos', $torah_trad[0]);
          $parashat_array['parashat_torah'] = $parashat_torah;
          break;

        case 'Numbers':
          $parashat_torah = str_replace('Numbers', 'Bamidbar/Números', $torah_trad[0]);
          $parashat_array['parashat_torah'] = $parashat_torah;
          break;

        case 'Deuteronomy':
          $parashat_torah = str_replace('Deuteronomy', 'Devarim/Deuteronômio', $torah_trad[0]);
          $parashat_array['parashat_torah'] = $parashat_torah;
          break;
